...
export function split1MXAudio(fileName, destPath) {
  ...
  let outPath = destPath,
    splitAudioCMD = `${ffmpeg} -i "${fileName}" -filter_complex 'channelsplit=channel_layout=mono[FC]' -acodec pcm_s24le -write_bext 1 -map '[FC]' "${outPath}\\${outNNE}.${outExt}"`
...
export function split2MXAudio(fileName, destPath) {
  const outNNE = path.parse(fileName).name,
    outExt = path.parse(fileName).ext.substring(1)
  let outPath = destPath,
    splitAudioCMD = `${ffmpeg} -i "${fileName}" -filter_complex 'channelsplit=channel_layout=stereo[FL][FR]' -acodec pcm_s24le -write_bext 1 -map '[FL]' "${outPath}\\${outNNE}.L.${outExt}" -acodec pcm_s24le -write_bext 1 -map '[FR]' "${outPath}\\${outNNE}.R.${outExt}"`
